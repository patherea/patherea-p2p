...
def my_load_checkpoint(model, filename, map_location=None, strict=False, logger=None):
    ...
    Returns:
        ...
        list: List of ViT (backbone) param names to optinally freeze.
    """
    ...
    if hasattr(model, "module"):
        vit_keys = load_state_dict(model.module, state_dict, strict, logger)
    else:
        vit_keys = load_state_dict(model, state_dict, strict, logger)
    return checkpoint, vit_keys
