...
from p2p.models.backbones.utils.mlp import Mlp
from p2p.models.backbones.utils.my_checkpoint import (
    my_load_checkpoint as load_checkpoint,
)
from p2p.models.backbones.utils.swiglu_ffn import SwiGLUFFNFused
...
class TIMMVisionTransformer(BaseModule):
    ...
    def __init__(
        ...
        if pretrained:
            backbone_keys = self.init_weights(pretrained)
            print(f"Backbone params: {backbone_keys}")
        if freeze_backbone and pretrained:
            for name, param in self.named_parameters():
                if name in backbone_keys:
                    print(f"Freezing backbone layer: {name}")
                    param.requires_grad = False

    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            _, backbone_keys = load_checkpoint(
